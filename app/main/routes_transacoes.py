...
def listar_transacoes(ano, mes):
    user_id = current_user.id; form = TransacaoForm()
    ...
    return render_template('main/transacoes.html', transacoes=transacoes_do_mes, form=form, user_categories=user_categories, nav_mes_display=mes_display, nav_mes_anterior=mes_anterior, nav_mes_seguinte=mes_seguinte)
...
def ignorar_ocorrencia():
    transacao_id = request.form.get('transacao_id', type=int); data_str = request.form.get('data_excecao')
    if not transacao_id or not data_str: flash('Informações inválidas para ignorar a transação.', 'danger'); return redirect(request.referrer or url_for('main.index'))
    data_excecao = datetime.strptime(data_str, '%Y-%m-%d').date()
    regra = db.session.get(Transacao, transacao_id)
    if not regra or regra.user_id != current_user.id: flash('Acesso negado.', 'danger'); return redirect(request.referrer)
    ...
    if not excecao_existente:
        nova_excecao = ExcecaoTransacao(transacao_id=transacao_id, data_excecao=data_excecao, user_id=current_user.id)
        db.session.add(nova_excecao); db.session.commit(); flash('Ocorrência ignorada com sucesso.', 'info')
...
def reativar_ocorrencia():
    transacao_id = request.form.get('transacao_id', type=int); data_str = request.form.get('data_excecao')
    if not transacao_id or not data_str: flash('Informações inválidas para reativar a transação.', 'danger'); return redirect(request.referrer or url_for('main.index'))
    data_excecao = datetime.strptime(data_str, '%Y-%m-%d').date()
    ...
    if excecao:
        db.session.delete(excecao); db.session.commit(); flash('Ocorrência reativada com sucesso.', 'success')
    else: flash('Não foi possível encontrar a ocorrência para reativar.', 'warning')
    return redirect(request.referrer or url_for('main.transacoes_redirect'))
...
def gerenciar_recorrencias():
    ...
    regras = Transacao.query.filter(Transacao.user_id == current_user.id, Transacao.recorrencia != None).order_by(Transacao.data.desc()).all()
