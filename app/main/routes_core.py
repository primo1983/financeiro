...
@login_required
def index():
    user_id = current_user.id; hoje = date.today(); form = TransacaoForm()
    ...
    for i in range(2, -1, -1):
        ...
        bar_chart_labels.append(mes_ref.strftime('%b/%y').capitalize()); bar_chart_receitas.append(resumo_barra['total_receitas']); bar_chart_despesas.append(resumo_barra['total_despesas'])
    ...
    return render_template('main/index.html', form=form, user_categories=user_categories, 
        saldo_atual=saldo_atual, receita_mes=resumo_mensal['total_receitas'], despesa_mes=resumo_mensal['total_despesas'], saldo_mes=resumo_mensal['saldo'],
        ultimas_10_transacoes=transacoes_ate_hoje[:10], pie_chart_labels=pie_chart_labels, pie_chart_valores=pie_chart_valores, 
        bar_chart_labels=bar_chart_labels, bar_chart_receitas=bar_chart_receitas, bar_chart_despesas=bar_chart_despesas, mostrar_saldos=current_user.mostrar_saldos)

@main_bp.route('/toggle-saldo-visibility', methods=['POST'])
@login_required
def toggle_saldo_visibility():
    user = db.session.get(Usuario, current_user.id); user.mostrar_saldos = not user.mostrar_saldos; db.session.commit()
