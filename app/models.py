...
class Usuario(db.Model, UserMixin):
    ...
    # Adiciona a relação com a nova tabela de exceções
...
# --- NOVA TABELA PARA LIDAR COM EXCEÇÕES ---
class ExcecaoTransacao(db.Model):
    __tablename__ = 'excecao_transacao'
    id = db.Column(db.Integer, primary_key=True)
    # A data da ocorrência que foi ignorada
    data_excecao = db.Column(db.Date, nullable=False)
    # Chaves estrangeiras para saber a qual regra e a qual usuário a exceção pertence
    transacao_id = db.Column(db.Integer, db.ForeignKey('transacoes.id'), nullable=False)
    user_id = db.Column(db.Integer, db.ForeignKey('usuarios.id'), nullable=False)
    # Garante que não se pode ignorar a mesma ocorrência duas vezes
